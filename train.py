...
def main():
    ...
    try:
        ...
        should_run = (args.mode == 'single' and cfg.SHOULD_TRAIN_SINGLE_RUN) or (args.mode == 'sweep' and cfg.SHOULD_TUNE)
        if should_run:
        ...
        if args.mode == 'single':
            if cfg.SHOULD_TRAIN_SINGLE_RUN and raw_text:
                ...
                logger.info("Single training is disabled in config or mode doesn't match. Exiting.")
        elif args.mode == 'sweep':
            if cfg.SHOULD_HYPERPARAMETER_SEARCH and raw_text:
                run_hyperparameter_search(raw_text, tokenizer, cfg)
            else:
                logger.info("Hyperparameter tuning is disabled in config or mode doesn't match. Exiting.")
