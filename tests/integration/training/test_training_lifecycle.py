...
class TestTrainingResumeAndRestart:
    """Tests for training resume behavior after completion."""
    
    @pytest.mark.integration
    def test_completed_run_is_not_retrained_on_restart(
        self, training_config_for_lifecycle_tests, sample_training_data, mock_tokenizer
    ):
        """
        Test that completed training runs are not retrained on restart.
        Covers: Resume behavior matches single run expectations.
        """
        config = training_config_for_lifecycle_tests
        ...
        with patch("wandb.init") as mock_wandb_init, \
            ...
            mock_run = MagicMock()
            mock_run.id = "completed-test-id"
            mock_run.name = "completed_test"
            mock_wandb_init.return_value = mock_run
            mock_wandb_run.id = "completed-test-id"
            mock_wandb_run.name = "completed_test"
        ...
        with patch("wandb.init") as mock_wandb_init_2, \
             ...
             patch("wandb.finish") as mock_wandb_finish_2, \
             patch("wandb.run") as mock_wandb_run_2, \
             patch("wandb.save") as mock_wandb_save_2, \
             patch("wandb.log_artifact") as mock_wandb_log_artifact_2, \
             patch("wandb.Artifact") as mock_artifact_2, \
             patch("src.training.manager.wait_for_artifact_upload", return_value=True), \
             patch("src.training.utils.train_loop") as mock_train_loop:
            
            mock_run_2 = MagicMock()
            mock_run_2.id = "completed-test-id"
            mock_run_2.name = "completed_test"
            mock_wandb_init_2.return_value = mock_run_2
            mock_wandb_run_2.id = "completed-test-id"
            mock_wandb_run_2.name = "completed_test"
            
            # Attempt to run again
            run_single_training(sample_training_data, mock_tokenizer, config)
            
            # Training loop should NOT be called for completed run
            mock_train_loop.assert_not_called()
...
class TestTrainingSmoke:
    ...
    @pytest.mark.integration
    def test_checkpoint_cleanup_occurs_after_completion(
        self, training_config_for_lifecycle_tests, sample_training_data, mock_tokenizer
    ):
        """
        Test that temporary artifacts are cleaned up post-training.
        Covers: Temporary artifacts (checkpoints, logs) are cleaned up post-training.
        """
        config = training_config_for_lifecycle_tests
        config.DELETE_LATEST_CHECKPOINT_ON_COMPLETION = True
        ...
        with patch("wandb.init") as mock_wandb_init, \
            ...
            mock_run = MagicMock()
            mock_run.id = "cleanup-test-id"
            mock_run.name = "cleanup_test"
            mock_wandb_init.return_value = mock_run
            mock_wandb_run.id = "cleanup-test-id"
            mock_wandb_run.name = "cleanup_test"
            ...
            # Check that best checkpoint exists but latest might be cleaned up
            checkpoint_files = os.listdir(config.MODEL_CHECKPOINTS_DIR)
            best_checkpoint_files = [f for f in checkpoint_files if "_best.pt" in f]
            assert len(best_checkpoint_files) > 0, "Best checkpoint should exist"
            
            # Latest checkpoint cleanup is acceptable based on configuration 