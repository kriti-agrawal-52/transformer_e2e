...
def plot_and_log_loss(train_losses, val_loss_dict, val_check_every, last_step, b_s, c_w, l_r, cfg, run_id):
        ...
        if len(train_losses) >= val_check_every:
            smoothed = [
                ...
                for i in range(
                    0, len(train_losses) - val_check_every + 1, val_check_every
                )
            ]
            steps_s = list(
                range(val_check_every, len(train_losses) + 1, val_check_every)
            )[:len(smoothed)]
            plt.plot(steps_s,smoothed, label=f"Smoothed Training ({val_check_every})", color="blue",lw=2)
        ...
        plot_path = os.path.join(cfg.LOSS_PLOT_DIRECTORY, f"loss_run_{run_id}_bs{b_s}_cw{c_w}_lr{l_r:.0e}.png"
        )  # this ensures that plots for various hyperparameter search runs are saved separately and are not overwritten
        plt.savefig(plot_path)

        wandb.log({"loss_curve": wandb.Image(plot_path)}, step=last_step)  # logging figures to weights and biases registry
        logger.info(f"Loss curve saved to {plot_path} and logged.")
        plt.close()

    
@torch.no_grad()
def evaluate_validation_loss(model, prep_obj, device, split="validation", eval_iters=20):
...
def train_loop(
        ...
        if initial_optimizer_state:  # loads the state dict for optimizer if training was interrupted
        ...
        for step in range(start_step, steps + 1):  # we have defined it this way that in case our training run gets interrupted in between
            ...
            try:
                ...
                train_losses.append(loss.item())  # append loss value (without gradient attached) to the train_losses list
                ...
                if step % val_check_every == 0 or step == steps:
                    ...
                    save_checkpoint(step, model, optimizer, best_val_loss, latest_checkpoint_path)  # we are checkpointing the model here
                ...
                print(
                    f"ERROR during step {step}. Saving latest checkpoint and exiting."
                )
                save_checkpoint(
        ...
        plot_and_log_loss(train_losses, val_loss_dict, val_check_every, step, b_s, c_w, l_r, cfg, run_id)
