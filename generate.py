...
def get_model_config_from_wandb(run_id, project_name):
    ...
    try:
        logger.info(f"Attempting to fetch config for run_id: {run_id} and project: {project_name}")
        runs = wandb.runs(project_name, {'_id': run_id})
        if not runs: 
            logger.warning(f"Could not find run_id '{run_id}' in project '{project_name}'. Trying a broader search.")
            # broaden search to look for run_id in all projects
            runs = wandb(filters={'_id': run_id})
            
        if not runs:
            logger.error(f"Could not find any run with W&B run_id: {run_id}. "
                             f"Ensure you are logged in and the project/run_id is correct.")
            raise ValueError
        
        if len(runs) > 1:
            logger.warning(f"Found multiple runs with ID {run_id}. Using the first one: {runs[0].path}")
...
def main():
    parser = argparse.ArgumentParser(description="Generate text using a trained Transformer model via config file.")
    parser.add_argument(
        '--gen_config',
        type = str, 
    ...
    if not wandb_project:
        logger.error(f"WANDB_PROJECT not found in local config file: {generate_cfg.MAIN_CONFIG_PATH}")
        sys.exit(1)
    ...
    tokenizer_name = model_config.get("tokenizer_name", local_cfg_dict.get("TOKENIZER_NAME", "gpt2"))
    ...
    logger.info(f"Loading model state from local checkpoint: {generate_cfg.CHECKPOINT_PATH}")
    checkpoint = torch.load(generate_cfg.CHECKPOINT_PATH, map_location=generate_cfg.DEVICE)
