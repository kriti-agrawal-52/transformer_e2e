...
class PreprocessingTraining:
    ...
    def __init__(
        self, text: str, tokenizer: object, batch_size: int = 4, time_steps: int = 64
    ):
        ...
        self.all_token_ids = self.tokenizer.encode(
            self.text
        )  # it tokeizes and encodes are input text, so even repeated tokens such as hello hello would be encoded separately.
    ...
    def get_batch(self, split: str = "train"):
        ...
        ix = torch.randint(
            0, max_start_idx + 1, (self.batch_size,)
        )  # in pytorch the third argument in randint has to be a tuple, even if we want a 1D tensor
...
def setup_data_and_tokenizer(cfg):
    ...
    try:
        ...
        raw_text = "\n".join(
            dataset["train"]["text"]
        )  # concatenate all of the dataset into a single string
        if cfg.RAW_TEXT_LIMIT:
            raw_text = raw_text[
                : cfg.RAW_TEXT_LIMIT
            ]  # since we are training on a cpu, we can limit our dataset to cgf.RAW_TEXT_LIMIT character limit
            logger.info(
                f"Truncated {cfg.DATASET_NAME} to {len(raw_text)} characters for CPU training."
            )
    except Exception as e:
        logger.critical(
            f"Could not load dataset: {e}", exc_info=True
        )  # exc_info tells the logger to include the full traceback of the exception in the log message
