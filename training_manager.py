    ...
    def run(self):
        ...
        try:
            ...
            if start_step > self.params["steps"]:
                logger.warning(
                    f"Run {self.run_id} already completed {start_step - 1} steps. Skipping."
                )
            ...
            if (
                self.is_single_run
                and self.best_ckpt_path
                and os.path.exists(self.best_ckpt_path)
            ):
                run_post_training_eval(
                    model, prep, self.best_ckpt_path, self.cfg, self.device
                )

            if (
                (self.is_single_run or self.cfg.ALWAYS_LOG_ARTIFACTS)
                and self.best_ckpt_path
                and os.path.exists(self.best_ckpt_path)
            ):
