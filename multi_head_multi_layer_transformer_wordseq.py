...
def hyperparameter_search(raw_text, tokenizer, lrs=[1e-2], batch_sizes=[4], time_steps_list=[8]): # Renamed time_steps to time_steps_list
    ...
    for bs in batch_sizes:
        for ts in time_steps_list: # Use new name
            for lr in lrs:
                # --- W&B Initialization for EACH Tuning Run ---
                # wandb.init() here signifies the start of a new W&B run for this specific hyperparameter combination.
                # Each 'run' corresponds to a single model training phase/experiment with these parameters.
                wandb.init(
                    project="decoder only transformer training", # Keep the same project name
                    name=f"tune_bs{bs}_cw{ts}_lr{lr:.0e}",      # Dynamic run name for easy identification
                    notes=f"Hyperparameter tuning run with BS={bs}, CW={ts}, LR={lr}",
                    tags=["transformer", "wikitext2", "tuning", "pytorch"],
                    reinit=True # Allows re-initializing wandb in a loop for new runs
                )
                try:
                    ...
                    # Log ALL hyperparameters (tuned and fixed) to this specific run's config
                    wandb.config.update({
                        "batch_size": bs,
                        "context_window": ts,
                        "learning_rate": lr,
                        "embedding_dim": base_model_params['channel_dim'], 
                        "num_heads": base_model_params['num_heads'],
                        "num_layers": base_model_params['num_layers'],
                        "device": str(DEVICE),
                        "dataset_name": "WikiText-2 Raw", # Inherit from main context if applicable
                    ...
                    # Get the checkpoint path generated by train_loop based on THIS run's config
                    checkpoints_dir = "model_checkpoints"
                    checkpoint_filename = f"transformer_bs{bs}_cw{ts}_lr{lr:.0e}_best.pt"
                    checkpoint_path_for_artifact = os.path.join(checkpoints_dir, checkpoint_filename)
                    ...
                    logger.info(f"Completed run: bs={bs}, ts={ts}, lr={lr}, val_loss={val_loss:.4f}")
                     ...
                     # --- Log the best model from THIS specific tuning run as a W&B Artifact ---
                    # An Artifact is a versioned collection of files, allowing for model lineage tracking.
                    # This registers the model in the W&B Model Registry.
                    if os.path.exists(checkpoint_path_for_artifact):
                        model_artifact_tune = wandb.Artifact(
                            name=f"model-tune-bs{bs}-cw{ts}-lr{lr:.0e}",
                            type="model",
                            description=f"Best model from tuning run (BS={bs}, CW={ts}, LR={lr})"
                        )
                        model_artifact_tune.add_file(checkpoint_path_for_artifact)
                        wandb.log_artifact(model_artifact_tune)
                        logger.info(f"Tuning run model artifact logged: {model_artifact_tune.name}")
                    else:
                        logger.warning(f"No checkpoint found at {checkpoint_path_for_artifact} for tuning run. Skipping artifact logging.")
                except Exception as e:
                    logger.error(f"Error in tuning run (bs={bs}, ts={ts}, lr={lr}): {e}", exc_info=True)
                finally:
                    # --- wandb.finish() for EACH Tuning Run ---
                    # This is crucial to properly close each individual run and sync its data.
                    wandb.finish() 
    return results

if __name__ == '__main__':
    
    # Define a control variable for switching between standard training and hyperparameter tuning
    should_tune = False 
    # Common variables that might be needed by both paths
    raw_text = None
    tokenizer = None
    ...
    try:        
        ...
        try:
            ...
            # Concatenate all training text into a single string
            raw_text = "\n".join(dataset['train']['text'])
            logger.info("Successfully loaded WikiText-2 dataset.")
            # It's a good idea to limit the training data size if you're on CPU
            # For CPU, consider using a smaller subset of WikiText-2, e.g., the first 1M characters
            # raw_text = raw_text[:1_000_000] 
            # logger.info(f"Truncated WikiText-2 to {len(raw_text)} characters for CPU training.")
        except Exception as e:
            ...
            exit() # terminating script execution immediately
        ...
        # --- W&B Initialization for the MAIN Training Run ---
        # wandb.init() here signifies the start of a W&B run for the standard training phase.
        # This run will track metrics and configurations for a single, fixed-hyperparameter training.
        if not should_tune: 
            wandb.init(
                project = "decoder only transformer training", 
                ...
                reinit=True # Allows re-initializing if needed in complex scripts, though primarily used by inner loops.
            )
            # Log ALL hyperparameters (fixed) to W&B config for this main run
            wandb.config.update({
                "batch_size": batch_s,
                "context_window": time_s,
                "embedding_dim": channel_d,
                "num_heads": num_h,
                "num_layers": num_l,
                "learning_rate": learning_r,
                "training_steps": training_steps,
                "validation_check_every": val_check,
                "early_stopping_patience": train_patience,
                "device": str(DEVICE),
                "dataset_size_chars": len(raw_text),
                "dataset_name": "WikiText-2 Raw",
        ...
        if should_train:
            ...
            # The checkpoint path uses the dynamic filename from wandb.config, which is correct.
            checkpoint_filename_main_load = f"transformer_bs{batch_s}_cw{time_s}_lr{learning_r:.0e}_best.pt"
            checkpoints_dir_main_load = "model_checkpoints" 
            checkpoint_path_for_load = os.path.join(checkpoints_dir_main_load, checkpoint_filename_main_load)
            
            try: 
                # This assumes the main run also saved a checkpoint to this path
                # based on its own config
                model.load_state_dict(torch.load(checkpoint_path_for_load, map_location=DEVICE))
                logger.info("Loaded best model from checkpoint for final evaluations.")
            except FileNotFoundError:
                logger.warning(f"Checkpoint file {checkpoint_path_for_load} not found after training. Using current model state for evaluations.")
            except Exception as e:
        ...
        # --- Save Final Trained Model as a W&B Artifact for the MAIN Run ---
        # Using wandb.Artifact explicitly registers the model in the W&B Model Registry.
        # This is done once after the main training run completes.
        if os.path.exists(checkpoint_path_for_artifact_main): 
            model_artifact_main = wandb.Artifact(
                name=f"model-{dataset_name_main}-bs{batch_size_main}-cw{context_window_main}-lr{learning_rate_main:.0e}",
                type="model",
                description=f"Decoder-only Transformer trained on {dataset_name_main} with HPs: BS={batch_size_main}, CW={context_window_main}, LR={learning_rate_main}"
            )
            ...
            logger.warning(f"Could not log main run artifact, checkpoint file not found at {checkpoint_path_for_artifact_main}. Ensure a checkpoint was saved.")
    ...
    finally: # Ensure the main run is finished if it was initialized
        # wandb.finish() here signifies the end of the W&B run initiated above.
        # This ensures all data is synced and the run is finalized.
        if not should_tune and wandb.run:
            wandb.finish()
     
        
    # == Optional: Hyperparameter Search Execution ==
    should_tune = True # set to False to turn off hyperparameter search
    try:
        if should_tune:
            logger.info("Starting hyperparameter search...")
            # If a main run was previously initialized and not finished (e.g., if should_tune was changed midway),
            # ensure it's finished before starting new runs for the sweep.
            if wandb.run: # Check if there's an active run from previous execution path
                wandb.finish()
                logger.info("Finished previous W&B run before starting hyperparameter search.")
            
            # Ensure raw_text and tokenizer are loaded, as the main run path might have been skipped
            if raw_text is None or tokenizer is None:
                logger.error("raw_text or tokenizer not defined for hyperparameter search. Attempting to reload.")
                try:
                    dataset = load_dataset("wikitext", "wikitext-2-raw-v1")
                    raw_text = "\n".join(dataset['train']['text'])
                    tokenizer_name = "gpt2"
                    ...
                    if tokenizer.pad_token is None:
                        tokenizer.pad_token = tokenizer.eos_token
                    logger.info("Reloaded dataset and tokenizer for hyperparameter search.")
                except Exception as e:
                    logger.critical(f"Could not reload dataset/tokenizer for HP search: {e}", exc_info=True)
                    print("Error: Could not reload dataset/tokenizer for hyperparameter search. Skipping.")
                    # If data cannot be loaded, it's safer to exit or skip tuning
                    return 
