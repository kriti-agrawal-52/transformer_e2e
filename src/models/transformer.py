...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        B, T, C = (
            x.shape
        )  # B: batch size, T: time_steps/context_window, C: embedding_dimension
        H = self.num_heads
        D = self.head_dim
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, channel_dim, num_heads, context_window, dropout_rate=0.2):
        ...
        self.ln2 = nn.LayerNorm(
            channel_dim
        )  # input to feed forward network is normalised
...
class TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        self.pos_emb_dropout = nn.Dropout(
            pos_emb_dropout_val
        ...
        if (
            final_dropout_multiplier is not None
            and final_dropout_multiplier != 1
            and num_layers > 1
        ):
            # calculate final dropout rate, capped by max_dropout_val
            # allows scaling up (multiplier > 1) or scaling down (multiplier < 1)
            final_layer_dropout = min(
                initial_layer_dropout * final_dropout_multiplier, max_dropout_val
            )
            final_layer_dropout = max(
                0.0, final_layer_dropout
            )  # Ensure dropout is not negative

            # Linearly interpolate dropout rates from initial to deep most layer ie final layer
            layer_dropout_rates = [
                initial_layer_dropout
                + (final_layer_dropout - initial_layer_dropout) * (i / (num_layers - 1))
                for i in range(num_layers)
            ]
            logger.info(
                f"Linearly scaling dropout from {initial_layer_dropout:.3f} to {final_layer_dropout:.3f} over {num_layers} layers. Result: {[float(f'{dr:.3f}') for dr in layer_dropout_rates]}"
            )
        else:
            # Use the same initial_layer_dropout for all layers
            layer_dropout_rates = [initial_layer_dropout] * num_layers
            logger.info(
                f"Using uniform dropout rate of {initial_layer_dropout} for all {num_layers} layers."
            )
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                TransformerBlock(
                    ...
                    num_heads,
                    context_window,
                    dropout_rate=layer_dropout_rates[i],
                )  # Pass the specific dropout rate for this layer
                for i in range(num_layers)
            ]
        )
    ...
    def forward(self, x_indices, targets=None):
        ...
        pos_indices = torch.arange(
            T, device=x_indices.device
        )  # creates a 1D tensor starting from 0 to upto T-1. x_indices tensor already exists. x_indices.device retrieves the device where the tensor stored in memory. so, the newly created tensor pos_indices is on the same device as x_indices
