...
def generate(self, input_ids, max_tokens_ahead, temperature, top_k, device):
    ...
    with torch.no_grad():
        for _ in range(max_tokens_ahead):
            ...
            logits, _ = self(cond)  # forward the cond vector through the entire model, and we get logits and loss, for generation loss
            ...
            if top_k is not None and top_k > 0:
                v, _ = torch.topk(last, min(top_k, last.size(-1)))  # identifies the top_k highest logit values (v) and their indices
                last[last < v[:, [-1]]] = -float("Inf") # v[:, [-1]]: This specifically extracts the k-th highest logit value.
            ...
            probs = F.softmax(last, dim=-1)  # When softmax is applied, -float('Inf') becomes 0 probability
            ...
            if temperature == 0:  # Deterministic: take the most probable token only
                ...
                next_t = torch.multinomial(probs, num_samples=1)  # getting one token from all of the probabilities
